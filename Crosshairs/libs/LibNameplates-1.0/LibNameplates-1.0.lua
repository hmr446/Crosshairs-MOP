...
do
	...
	function UnitIterator()
		rmem, step = GetNumGroupMembers(), 1
		if rmem == 0 then
...
function lib:CheckDefaultNameplateGUID(frame)
    if not self.nameplates[frame] then return end
    
    -- 通过姓名版关联的单位识别
    if frame.unit and UnitExists(frame.unit) then
        FoundPlateGUID(frame, UnitGUID(frame.unit), frame.unit, "DefaultPlate")
        return true
    end
    
    -- 通过目标对比识别
    if self:IsTarget(frame, true) then
        FoundPlateGUID(frame, UnitGUID("target"), "target", "DefaultTarget")
        return true
    end
end
...
do
    ...
    if healthBar and not self.healthOnValueChangedHooks[frame] then
        -- 添加安全检测
        if healthBar.HasScript and healthBar:HasScript("OnValueChanged") then
            self.healthOnValueChangedHooks[frame] = true
            healthBar:HookScript("OnValueChanged", ourHealthOnValueChanged)
        else
            debugPrint("HealthBar does not support OnValueChanged:", healthBar:GetObjectType())
        end
    end
end
end
...
function lib:IsTarget(frame, quick)
    ...
    if frame.UnitFrame and frame.UnitFrame.unit then
        return UnitIsUnit(frame.UnitFrame.unit, "target")
    end
	if not frame then return end
...
function lib:GetHealthBar(frame)
    ...
    if frame.UnitFrame and frame.UnitFrame.healthBar then
        return frame.UnitFrame.healthBar
    end
	if not frame then
		return
